...
Includes fix for Ask AI text input clearing.
"""
...
# Use Themer specific modules
import config as cfg
from styling import apply_styling # Themer's styling
import utils
import ai_core
...
st.set_page_config(
    layout="wide",
    page_title="AI Themer - Apex", # Updated Title
    page_icon="🏷️"
)
...
default_state_values = {
    ...
    # Add other keys if they have defaults
...
def clear_themer_app_state(clear_api_keys=False):
    ...
    keys_to_clear = [
        ...
        # Add any other specific keys used only by themer if needed
        "ai_qa_input_main_widget", # Clearing specific widget state key
    ]
    if clear_api_keys:
        keys_to_clear.extend([
            cfg.INITIALIZED_STATE_KEY, cfg.API_KEY_STATE_KEY, cfg.API_KEY_SOURCE_STATE_KEY,
            # Reset settings if clearing API key
    ...
    # Re-initialize essential keys after clearing
    if clear_api_keys:
        ...
        # Re-apply default settings if needed
        st.session_state[cfg.BATCH_SIZE_KEY] = 15
        st.session_state[cfg.SAMPLE_SIZE_KEY] = 100
        # ... other settings ...
        logging.info(f"Cleared {cleared_count} Themer keys and reset API/init/settings state.")
         ...
         st.session_state[cfg.ASSIGNMENT_DF_KEY] = None
         logging.info(f"Cleared {cleared_count} Themer data/state keys.")


# --- Logo Loading Function (from landing page) ---
# (Make sure this util function is appropriate or adapt if needed)
def get_base64_of_bin_file(bin_file):
    ...
    try:
        ...
        st.sidebar.warning(f"Logo file '{cfg.LOGO_FILENAME}' not found.")
...
with st.sidebar:
    ...
    logo_base64 = get_base64_of_bin_file(LOGO_PATH) # Use shared function name
    ...
    # Logo CSS is now in styling.py
    st.markdown(logo_html, unsafe_allow_html=True)

    st.header("⚙️ Settings & Controls")

    # --- API Key Handling Logic ---
    # Uses themer config state keys (cfg.INITIALIZED_STATE_KEY, etc.)
    # Uses themer utils.validate_api_key
    api_key_source = None
    if st.session_state.get(cfg.INITIALIZED_STATE_KEY, False) and st.session_state.get(cfg.API_KEY_STATE_KEY):
        st.success("✅ Initialized")
        api_key_source = st.session_state.get(cfg.API_KEY_SOURCE_STATE_KEY, "manual")
        ...
        try:
            if hasattr(st, 'secrets') and secret_key_name in st.secrets:
                secrets_api_key = st.secrets[secret_key_name]; api_key_source = "secrets"
                logging.info(f"Themer: Found API key in Secrets ('{secret_key_name}').")
        ...
        if api_key_source == "secrets" and secrets_api_key:
            is_valid, message = utils.validate_api_key(secrets_api_key) # Use themer utils
            if is_valid:
                ...
                st.success("✅ Initialized via Secrets!"); logging.info("Themer: Initialized via Secrets.")
                time.sleep(0.5); st.rerun()
            else:
                st.error(f"Secrets key invalid: {message}"); logging.error(f"Themer: Secrets key fail: {message}")
                api_key_source = "manual"
                st.session_state.pop(cfg.API_KEY_STATE_KEY, None); st.session_state.pop(cfg.INITIALIZED_STATE_KEY, None); st.session_state.pop(cfg.API_KEY_SOURCE_STATE_KEY, None)
        ...
        if not st.session_state.get(cfg.INITIALIZED_STATE_KEY, False):
            if api_key_source == "manual":
                ...
                if st.button("Initialize Themer", key=cfg.INIT_BUTTON_KEY, type="primary"): # Use themer specific key from config
                    if api_key_input:
                        is_valid, message = utils.validate_api_key(api_key_input) # Use themer utils
                        if is_valid:
                            st.session_state[cfg.API_KEY_STATE_KEY] = api_key_input
                            st.session_state[cfg.INITIALIZED_STATE_KEY] = True
                            st.session_state[cfg.API_KEY_SOURCE_STATE_KEY] = "manual"
                            st.success("✅ Initialized (Manual Key)!"); logging.info("Themer: Initialized via manual key.")
                            time.sleep(0.5); st.rerun()
                        else:
    ...
    # --- Reset Button ---
    st.sidebar.markdown("---")
    if st.sidebar.button("Reset API Key & Clear All Data", key=cfg.RESET_BUTTON_KEY): # Use themer reset key
        logging.warning("Themer: Reset button clicked.")
        clear_themer_app_state(clear_api_keys=True) # Use themer clear function
...
with input_tab:
    ...
    if st.button("🤖 Generate Themes & Descriptions", key="generate_themes_desc_main_btn", disabled=not can_generate, type="primary"):
        if can_generate:
            ...
            if actual_sample_size < len(responses_raw): st.info(f"Using sample of {actual_sample_size} responses."); try: responses_sample = random.sample(responses_raw, actual_sample_size) except ValueError as e: st.error(f"Sample error: {e}. Using all."); responses_sample = responses_raw
...
# (Keep existing Themer Assign Tab code - PASTE HERE)
...
# (Keep existing Themer Viz Tab code - PASTE HERE)
with viz_tab:
            ...
            if not user_question_stripped:
                ...
                with st.spinner("🧠 AI is thinking..."):
                    if not api_key:
                         ...
                         # Call the CORRECTED Q&A function from ai_core
                ...
                if ai_answer is not None:
                    ...
                    logging.error("Themer Q&A: ask_ai_about_data returned None.")
